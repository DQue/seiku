...
const 艦種 = ["装甲空母", "正規空母", "軽空母", "揚陸艦", "水上機母艦", "軽巡洋艦", "航空巡洋艦", "重巡洋艦", "航空戦艦", "戦艦", "潜水空母", "補給艦", "潜水母艦", "駆逐艦", "工作艦", "基地航空隊"];
const 艦種略称 = {
	"装甲空母": "装母",
	"正規空母": "航",
	"軽空母": "軽母",
...
const 艦種その他枠 = ["揚陸艦", "軽巡洋艦", "重巡洋艦", "戦艦", "潜水空母", "補給艦", "潜水母艦", "駆逐艦", "工作艦"];
const 艦娘データ = {
	"翔鶴": {
		"読み": "ショウカク",
		"データ": {
	...
	"鳳翔": {
		...
		"データ": {
			...
			"改二": { "スロット": [17, 18, 13, 1], "艦種": "軽空母", "改造": 2, "素火力": 53, "id": 894 },
			"改二戦": { "スロット": [17, 18, 4, 0], "艦種": "軽空母", "改造": 3, "素火力": 50, "id": 899, "表示艦種": "戦力投射母艦" }
		}
	},
	"飛鷹": {
		"読み": "ヒヨウ",
	...
	"あきつ丸": {
		"読み": "アキツマル",
		"データ": {
			"無印": { "スロット": [0, 0, 0], "艦種": "揚陸艦", "改造": 0, "id": 161 },
	...
	"大和": {
		...
		"データ": {
			...
			"改二": { "スロット": [4, 4, 4, 8, 2], "艦種": "戦艦", "改造": 2, "id": 911 },
			"改二重": { "スロット": [2, 2, 8, 22, 2], "艦種": "航空戦艦", "改造": 3, "id": 916 },
		}
	},
	"武蔵": {
		"読み": "ムサシ",
	...
	"Jean Bart": {
		"読み": "ジャンバール",
		"データ": {
			"改": { "スロット": [3, 3, 3, 3], "艦種": "戦艦", "改造": 1, "id": 724 }
		}
	},
	"基地航空隊": {
		"読み": "キチコウクウタイ",
	...
	"神威": {
		"読み": "カモイ",
		"データ": {
			"無印": { "スロット": [0, 0, 0], "艦種": "補給艦", "改造": 0, "id": 162 },
	...
	"長鯨": {
		"読み": "チョウゲイ",
		"データ": {
			"無印": { "スロット": [1, 1], "艦種": "潜水母艦", "改造": 0, "id": 635 },
			"改": { "スロット": [2, 2, 2], "艦種": "潜水母艦", "改造": 1, "id": 640 },
		}
	},
	"Conte di Cavour": {
		"読み": "カブール",
		"データ": {
			"nuovo": { "スロット": [1, 1, 3, 4], "艦種": "戦艦", "改造": 2, "id": 879, "名前変更": "Conte di Cavour nuovo" },
		}
	},
	"Victorious": {
		"読み": "ヴィクトリアス",
	...
	"Langley": {
		"読み": "ラングレー",
		"データ": {
			"無印": { "スロット": [23, 7, 2], "艦種": "軽空母", "改造": 0, "id": 925, "素火力": 32 },
			"改": { "スロット": [24, 16, 4, 1], "艦種": "軽空母", "改造": 1, "id": 930, "素火力": 40 },
		}
	},
	"Ranger": {
		"読み": "レンジャー",
		"データ": {
			"無印": { "スロット": [24, 24, 24, 4], "艦種": "正規空母", "改造": 0, "id": 931, "素火力": 27 },
			"改": { "スロット": [28, 30, 24, 4], "艦種": "正規空母", "改造": 1, "id": 723, "素火力": 47 },
		}
	},
	"Samuel B.Roberts": {
		"読み": "サミュエル・ビー・ロバーツ",
		"データ": {
			"Mk.II": { "スロット": [1, 1, 1], "艦種": "駆逐艦", "改造": 2, "id": 920, "素火力": 52, "名前変更": "Samuel B.Roberts Mk.II", "表示艦種": "護衛駆逐艦" },
		}
	},
	"天龍": {
		"読み": "テンリュウ",
		"データ": {
			"改二": { "スロット": [1, 1, 1], "艦種": "軽巡洋艦", "改造": 2, id: 477, "素火力": 64 }
		}
	},
	"龍田": {
		"読み": "タツタ",
		"データ": {
			"改二": { "スロット": [1, 1, 1], "艦種": "軽巡洋艦", "改造": 2, id: 478, "素火力": 64 }
		}
	},
	"宗谷": {
		"読み": "ソウヤ",
		"データ": {
			"南極観測船": { "スロット": [1, 2, 3], "艦種": "補給艦", "改造": 2, id: 650, "名前変更": "宗谷(南極観測船)", "表示艦種": "南極観測船" }
		}
	},
	"明石": {
		"読み": "アカシ",
		"データ": {
			"無印": { "スロット": [0, 0, 0], "艦種": "工作艦", "改造": 0, id: 182 },
			"改": { "スロット": [0, 0, 0, 0], "艦種": "工作艦", "改造": 1, id: 187 }
		}
	},
}

const 装備種 = ["装備なし", "艦上戦闘機", "陸上攻撃機", "大型陸上機", "局地戦闘機", "陸軍戦闘機", "艦上偵察機", "陸上偵察機", "水上偵察機", "大型飛行艇", "噴式戦闘爆撃機", "艦上爆撃機", "艦上攻撃機", "水上爆撃機", "水上戦闘機", "回転翼機", "対潜哨戒機"];
const 艦戦データ = {
	...
	"零式艦戦64型(制空戦闘機仕様)": { "対空値": 9, "種類": "艦上戦闘機", "半径": 5, "コスト": 6, "id": 486 },
	...
	"F4U-2 Night Corsair": { "対空値": 9, "火力": 2, "種類": "艦上戦闘機", "夜間航空機": true, "半径": 6, "id": 473 },
	...
	"Mosquito FB Mk.VI": { "対空値": 5, "種類": "陸上攻撃機", "半径": 7, "id": 479 },
	...
	"紫電改(三四三空) 戦闘701": { "対空値": 11, "種類": "局地戦闘機", "迎撃": 4, "対爆": 2, "半径": 4, "コスト": 6, "id": 264 },
	...
	"Mosquito PR Mk.IV": { "対空値": 0, "種類": "陸上偵察機", "索敵値": 9, "半径": 9, "id": 480 },
	...
	"零式艦戦64型(熟練爆戦)": { "対空値": 7, "爆装": 6, "種類": "艦上爆撃機", "半径": 5, "コスト": 6, "id": 487 },
	"SBD": { "対空値": 2, "爆装": 6, "火力": 1, "種類": "艦上爆撃機", "半径": 4, "コスト": 5, "id": 195 },
	"F4U-1D": { "対空値": 7, "爆装": 7, "火力": 1, "種類": "艦上爆撃機", "半径": 6, "コスト": 9, "id": 233 },
	"F4U-4": { "対空値": 10, "爆装": 6, "火力": 2, "種類": "艦上爆撃機", "半径": 6, "id": 474 },
	"F4U-7": { "対空値": 10, "爆装": 11, "火力": 3, "種類": "艦上爆撃機", "半径": 6, "id": 476 },
	...
	"AU-1": { "対空値": 9, "爆装": 12, "火力": 3, "種類": "艦上爆撃機", "半径": 6, "id": 475 },
	...
	"Mosquito TR Mk.33": { "対空値": 5, "雷装": 8, "種類": "艦上攻撃機", "半径": 7, "id": 481 },
	...
	"試製 夜間瑞雲(攻撃装備)": { "対空値": 4, "爆装": 10, "種類": "水上爆撃機", "半径": 4, "コスト": 8, "id": 490, "夜間航空機": true },
	...
	"強風改二": { "対空値": 6, "種類": "水上戦闘機", "半径": 3, "コスト": 6, "id": 485 },

	//回転翼機
	"カ号観測機": { "対空値": 0, "種類": "回転翼機", "半径": 1, "コスト": 2, "id": 69 },
	"オ号観測機改": { "対空値": 0, "種類": "回転翼機", "半径": 1, "コスト": 3, "id": 324 },
	"オ号観測機改二": { "対空値": 0, "種類": "回転翼機", "半径": 1, "コスト": 4, "id": 325 },
	"S-51J": { "対空値": 0, "種類": "回転翼機", "半径": 2, "コスト": 5, "id": 326 },
	"S-51J改": { "対空値": 0, "種類": "回転翼機", "半径": 2, "コスト": 5, "id": 327 },

	//対潜哨戒機
	"三式指揮連絡機改": { "対空値": 0, "種類": "対潜哨戒機", "半径": 2, "コスト": 4, "id": 451 },
	"三式指揮連絡機(対潜)": { "対空値": 0, "種類": "対潜哨戒機", "半径": 2, "コスト": 3, "id": 70 },
	"一式戦 隼II型改(20戦隊)": { "対空値": 6, "種類": "対潜哨戒機", "半径": 4, "コスト": 4, "id": 489 },
	"一式戦 隼III型改(熟練/20戦隊)": { "対空値": 7, "種類": "対潜哨戒機", "半径": 4, "コスト": 4, "id": 491 },
...
const 雑データ = {
	...
	"短縮": {
		"装備種": {
			"装備なし": "なし", "艦上戦闘機": "艦戦", "陸上攻撃機": "陸攻", "大型陸上機": "重爆",
			"局地戦闘機": "局戦", "陸軍戦闘機": "陸戦", "艦上偵察機": "艦偵", "陸上偵察機": "陸偵",
			"水上偵察機": "水偵", "大型飛行艇": "飛行艇", "噴式戦闘爆撃機": "噴戦", "艦上爆撃機": "艦爆",
			"艦上攻撃機": "艦攻", "水上爆撃機": "水爆", "水上戦闘機": "水戦",
			"回転翼機": "ヘリ", "対潜哨戒機": "哨戒機"
		},
	},
	"海域略称": {/*
