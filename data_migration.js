function データ移行(O){
	...
	for(var I=0; I<O.table.length; I++){
		...
		for(var i=0; i<移行用データ.kanmusu.length; i++){ //艦娘
			if(d.kanmusu===移行用データ.kanmusu[i].前.名前 && (移行用データ.kanmusu[i].前.改造===undefined || 移行用データ.kanmusu[i].前.改造===d.kaizou)){
				d.kanmusu=移行用データ.kanmusu[i].後.名前;
				d.kaizou=移行用データ.kanmusu[i].後.改造;
				L.push({"前":`${移行用データ.kanmusu[i].前.名前} ${移行用データ.kanmusu[i].前.改造}`,"後":`${d.kanmusu} ${d.kaizou}`});
			}
...
const データ移行通知を表示=(L)=>{
	if($("データ移行通知")){
		...
		n.childNodes[2].appendChild(ct("更新によって利用できなくなったデータを変更しました："))
...
//		{"前":"烈風改","後":"烈風 一一型"},
	...
	kanmusu:[
		...
		{"前":{"名前":"基地航空隊","改造":"無印"},"後":{"名前":"基地航空隊","改造":"出撃"}},
		{"前":{"名前":"基地航空隊","改造":"2016春"},"後":{"名前":"基地航空隊","改造":"出撃"}},
	],
	teki:[
		{"前":"軽母ヌ級","後":"ヌ"},
		{"前":"軽母ヌ級elite","後":"ヌe"},
